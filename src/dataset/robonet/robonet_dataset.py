...
from src.utils.camera_calibration import world_to_camera_dict, cam_intrinsics_dict, camera_to_world_dict
import ipdb
...
class RoboNetDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        with h5py.File(hdf5_path, "r") as hf:
            ...
            low, high = self._load_bounds(hf, robot_viewpoint, idx)
    ...
    def _load_bounds(self, file_pointer, robot_viewpoint, idx):
        ...
        if "camera" in self._config.preprocess_action:
            ...
            bounding_box = np.concatenate([bounding_box, np.ones((bounding_box.shape[0], 1))], 1).T
    ...
    def _preprocess_actions(self, states, actions, low, high, idx):
        ...
        if strategy == "camera_raw":
            ...
            actions = self._make_camera_actions(states, actions, world2cam, cam2world, low, high)
    ...
    def _make_camera_actions(self, states, actions, w_to_c, c_to_w, low, high):
        ...
        next_eef_pos = np.concatenate([next_eef_pos, np.ones((next_eef_pos.shape[0], 1))], 1).T
