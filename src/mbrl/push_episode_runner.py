...
from src.cem.push.cem import CEMPolicy
...
from src.env.robotics.locobot_push_env import LocobotPushEnv
...
class EpisodeRunner(object):
    ...
    def __init__(self, config) -> None:
        ...
        self._env = LocobotPushEnv(config)
    ...
    def run_episode(self, ep_num, demo_name, demo_path):
        ...
        if cfg.cyclegan: # load goals from source robot
            source_demo_path = "/home/edward/roboaware/demos/locobot_push/med_push_0_1_f.hdf5"
            source_demo = self._load_demo(source_demo_path)
            goal_timesteps = DEMO_GOAL_LABELS["med_push_0_1_f.hdf5"]
            goals = self._extract_goals_from_demo(goal_timesteps, source_demo)
            goal_pos = goals[-1]["obj_qpos"][:3]
        else:
        ...
        max_timestep = 12
        start_timestep = 0
        initial_state = {"qpos": demo["qpos"][start_timestep], "obj_qpos": demo["obj_qpos"][start_timestep], "eef_states": demo["eef_states"][start_timestep]}

        if cfg.cyclegan:
            initial_state["obj_qpos"] = source_demo["obj_qpos"][start_timestep]
            # initialize the target robot with source robot state
            initial_state["eef_states"] = source_demo["eef_states"][start_timestep]
        ...
        while True:
            ...
            # pad ac from 2d to 4d
            ac = np.concatenate([ac, [0,0]])
            print("executing", ac)
    ...
    def run(self):
        ...
        self._logger.info(f"Final Dist {(final_dist / len(all_files)):.4f}")
...
if __name__ == "__main__":
    ...
    demo_path = f"/home/edward/roboaware/demos/locobot_push/{demo_name}"
    config, _ = argparser()

    # env
    config.modified = True
    config.object_demo_dir = "/home/edward/roboaware/demos/fetch_push_demos"
    ...
    # Locobot Push demos
    DEMO_GOAL_LABELS = {
        "med_push_0_1_f.hdf5": [5,12],
        "med_push_0_6_f.hdf5": [5,12],
    }
    ...
    config.world_cost_success = 1200
    # config.world_cost_success = 0.4
