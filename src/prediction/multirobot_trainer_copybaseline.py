...
class MultiRobotPredictionTrainer(object):
    ...
    def _compute_epoch_metrics(self, data_loader, name):
        ...
        for data, robot_name in tqdm(data_loader, f"computing {name} epoch"):
            # transpose from (B, L, C, W, H) to (L, B, C, W, H)
            frames, robots, actions, masks = data
            frames = frames.transpose_(1, 0).to(self._device)
            robots = robots.transpose_(1, 0).to(self._device)
        ...
        # plot horizontal line for baseline metrics
        return avg_loss
    ...
    def train(self):
        """Compute metrics for train, test, transfer datasets
        """
        cf = self._config
    ...
    def plot(self, data, epoch, name):
        ...
        for s in range(nsample):
            ...
            for i in range(1, cf.n_eval):
                ...
                if self._config.reconstruction_loss == "dontcare_mse":
                    x_pred_copy = x_pred.clone()
                    self._zero_robot_region(mask[i], x_pred_copy)
                if i < cf.n_past:
                    x_j = x_i
                    gen_seq[s].append(x_j)
                else:
                    x_j = x_pred
                    gen_seq[s].append(x_pred_copy)
